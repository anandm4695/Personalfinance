...
export function MonthlyReportModal({ metrics, state, selectedDate, onClose }: any) {
  ...
  const historicalNW = !isCurrentMonth ? (state.netWorthHistory || []).find((h: any) => h.month === ym) : null;
  ...
  const prevNW = historicalNWPrev ? (historicalNWPrev.netWorth ?? historicalNWPrev.net_worth ?? null) : null;
