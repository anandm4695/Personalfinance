    ...
    <div className="modal-backdrop" onClick={onCancel}>
      <div className="modal-panel" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 420 }}>
        <div className="modal-header">
          <div style={{ fontSize: 17, fontWeight: 800, letterSpacing: "-0.02em", color: "var(--t-ink)" }}>Confirm Action</div>
          <button className="modal-close-btn" onClick={onCancel} aria-label="Close"><X size={16} /></button>
        </div>
        <div className="modal-body">
          <p style={{ fontSize: 14, color: "var(--t-ink)", lineHeight: 1.6, marginBottom: 24 }}>{message}</p>
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <button style={cancelBtnStyle} onClick={onCancel}>Cancel</button>
            <button
              onClick={onConfirm}
              style={{ ...cancelBtnStyle, background: "#DC2626", border: "1px solid #DC2626", color: "#fff", fontWeight: 700 }}
            >
              Yes, delete
            </button>
          </div>
...
export function ToastStack({ toasts }: { toasts: { id: string; msg: string; type: string }[] }) {
  ...
  return ReactDOM.createPortal(
    <div style={{ position: "fixed", bottom: 32, right: 20, zIndex: 9999, display: "flex", flexDirection: "column", gap: 10, pointerEvents: "none" }}>
      {toasts.map((t) => {
