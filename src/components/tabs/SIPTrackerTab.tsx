...
export function SIPTrackerTab({ state, addItem, removeItem, updateItem, metrics }: any) {
  ...
  return (
    <div className="tab-content-enter">
        ...
        <>
          ...
          {fundTypeAlloc.length > 1 && (
            <Card style={{ marginBottom: 20, padding: "18px 20px" }}>
              <div style={{ fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase", color: THEME.muted, fontWeight: 800, marginBottom: 14 }}>
                Portfolio Allocation by Fund Type
              </div>
              <div style={{ display: "flex", height: 10, borderRadius: 99, overflow: "hidden", marginBottom: 16, gap: 3 }}>
                  ...
                  return (
                    <div key={type} style={{
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 10, color: THEME.muted }}>{fmtINRFull(amt)}/mo · {pct.toFixed(0)}%</div>
                      </div>
                    </div>
                  );
                })}
            ...
            <div style={{
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    padding: "4px 10px", borderRadius: 7, border: "none", fontSize: 11, fontFamily: "inherit",
                    fontWeight: sortBy === opt.key ? 800 : 600, cursor: "pointer",
          ...
          {sortedActive.length > 0 && (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))", gap: 16, marginBottom: sortedCompleted.length > 0 ? 28 : 28 }}>
                {sortedActive.map((sip: any) => (
                  <SIPCard key={sip.id} sip={sip} onEdit={() => setEditSip(sip)} onRemove={() => removeItem("sips", sip.id)} />
                ))}
              </div>
            </>
          )}
          ...
          <Card style={{ marginBottom: 28, padding: 24 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
              ...
              <div style={{
                ...
                background: `${THEME.accent}09`, padding: "6px 14px", borderRadius: 10,
      ...
      {show && (
        <SIPModal onClose={() => setShow(false)} onSave={(v: any) => { addItem("sips", v); setShow(false); }} />
      )}
      {editSip && (
        <SIPModal
          initial={editSip}
  ...
  return (
    <Card style={{ padding: "18px 20px", borderTop: `3px solid ${statusColor}`, position: "relative" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
            ...
            {sip.frequency && sip.frequency !== "monthly" && (
              <span style={{ fontSize: 9, fontWeight: 700, color: THEME.accent, background: `${THEME.accent}12`, border: `1px solid ${THEME.accent}25`, borderRadius: 4, padding: "2px 6px", textTransform: "uppercase" as const, letterSpacing: "0.06em" }}>
                {sip.frequency}
              </span>
            )}
            {sip.broker && (
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <div>
          <span style={{ fontSize: 22, fontWeight: 900, color: statusColor, fontVariantNumeric: "tabular-nums" }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: THEME.muted, marginBottom: 5, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: "0.08em" }}>
          <span>Progress</span>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
        {[
          ...
          <div key={k} style={{ display: "flex", flexDirection: "column", gap: 2, padding: "7px 10px", borderRadius: 9, background: `${color}09`, border: `1px solid ${color}22`, flex: "1 1 80px" }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color, fontVariantNumeric: "tabular-nums" }}>{v}</span>
          </div>
        ))}
      </div>
