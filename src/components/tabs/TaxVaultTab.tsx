...
import {
  ...
  Info, AlertTriangle, Download, Lightbulb, Award, Sparkles,
  TrendingDown, ChevronDown, ChevronUp, Zap, ArrowRight
} from "lucide-react";
...
const AddTaxPaymentModal = ({ onClose, onSave }: any) => {
  ...
  return (
    <Modal title="Record Tax Payment" onClose={onClose}>
      ...
      <ModalActions onSave={() => f.amount && onSave({ ...f, id: uid() })} onClose={onClose} saveLabel="Record Payment" />
    </Modal>
  );
};
...
const SlabBreakdownTable = ({ result, regime }: { result: any; regime: "new" | "old" }) => {
  ...
  return (
    <div>
      {/* Income → Taxable */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 4, marginBottom: 16, paddingBottom: 16, borderBottom: `1px dashed ${THEME.line}` }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: THEME.muted }}>Gross Annual Income</span>
        <span style={{ fontSize: 13, fontWeight: 800, color: THEME.ink, textAlign: "right" }}>{fmtINRFull(result.grossIncome)}</span>

        <span style={{ fontSize: 13, fontWeight: 600, color: THEME.muted }}>
          {`Standard Deduction${regime === "old" ? " (Sec 16(ia))" : ""}`}
        </span>
        <span style={{ fontSize: 13, fontWeight: 700, color: THEME.rust, textAlign: "right" }}>- {fmtINRFull(result.stdDed)}</span>

        {regime === "old" && result.extraDeds > 0 && (
          <>
            <span style={{ fontSize: 13, fontWeight: 600, color: THEME.muted }}>Other Deductions (80C, 80D, HRA…)</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: THEME.rust, textAlign: "right" }}>- {fmtINRFull(result.extraDeds)}</span>
          </>
        )}
        ...
        <span style={{ fontSize: 14, fontWeight: 900, color: accentColor, textAlign: "right" }}>{fmtINRFull(result.taxable)}</span>
      </div>

      {/* Slab rows */}
      <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 16 }}>
        {result.slabs.filter((s: any) => s.incomeInSlab > 0).map((slab: any, i: number) => (
          <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 110px 90px", gap: 8, alignItems: "center", padding: "8px 10px", borderRadius: 8, background: slab.rate === 0 ? `${THEME.sage}09` : `${accentColor}08`, border: `1px solid ${slab.rate === 0 ? THEME.sage : accentColor}18` }}>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: slab.rate === 0 ? THEME.sage : accentColor }}>{slab.label}</div>
              <div style={{ fontSize: 10, color: THEME.muted }}>{slab.range}</div>
            </div>
            <div style={{ textAlign: "right", fontSize: 12, color: THEME.muted, fontWeight: 600 }}>{fmtINRFull(slab.incomeInSlab)}</div>
            <div style={{ textAlign: "right", fontSize: 13, fontWeight: 800, color: slab.rate === 0 ? THEME.sage : THEME.ink }}>
      ...
      {/* Tax totals */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 6, padding: "12px 0", borderTop: `1px dashed ${THEME.line}` }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: THEME.muted }}>Tax on Total Income</span>
        <span style={{ fontSize: 13, fontWeight: 800, color: THEME.ink, textAlign: "right" }}>{fmtINRFull(result.tax + (result.rebateApplied ? result.rebateAmount : 0))}</span>

        {result.rebateApplied && (
          <>
            <span style={{ fontSize: 13, fontWeight: 600, color: THEME.sage }}>Section 87A Rebate</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: THEME.sage, textAlign: "right" }}>- {fmtINRFull(result.rebateAmount || (result.tax + result.rebateAmount))}</span>
          </>
        )}
        ...
        {result.surcharge > 0 && (
          <>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: THEME.rust, textAlign: "right" }}>+ {fmtINRFull(result.surcharge)}</span>
          </>
        )}

        <span style={{ fontSize: 13, fontWeight: 600, color: THEME.muted }}>Health & Education Cess (4%)</span>
        <span style={{ fontSize: 13, fontWeight: 700, color: THEME.muted, textAlign: "right" }}>+ {fmtINRFull(result.cess)}</span>

        <div style={{ gridColumn: "1/-1", height: 2, background: accentColor, borderRadius: 1, margin: "4px 0" }} />

        <span style={{ fontSize: 15, fontWeight: 900, color: THEME.ink }}>Net Tax Liability</span>
        <span style={{ fontSize: 15, fontWeight: 900, color: result.total === 0 ? THEME.sage : THEME.rust, textAlign: "right" }}>
          {result.total === 0 ? "₹0 (Nil)" : fmtINRFull(result.total)}
        </span>

        <span style={{ fontSize: 12, fontWeight: 600, color: THEME.muted }}>Effective Tax Rate</span>
...
export const TaxVaultTab: React.FC<TaxVaultTabProps> = ({ state, metrics, addItem, removeItem }) => {
  ...
  const inFY = (d: string) => d && d >= fyStartStr && d <= fyEndStr;
  ...
  const taxNewResult = useMemo(
    () => calcTaxNewByFY(annualIncome, fy),
    [annualIncome, fy]
  );
  ...
  const taxSavingTips = useMemo(() => {
    ...
    const maxEmpNPS80CCD2 = annualIncome > 0 ? annualIncome * (activeRegime === "new" ? 0.14 : 0.10) : 0;
    ...
    const ltcgGains = 0; // from capital gains section
  ...
  }, [deductions, annualIncome, taxOldResult.taxable, regimeVerdict, activeRegime, taxNewResult.effectiveRate]);

  /* ── Capital Gains computations (unchanged logic) ────────────── */
  const realizedGainsData = useMemo(() => {
    const fyStockSells = (state.stockSells || []).filter((s: any) => inFY(s.sellDate));
    const fyMfSells = (state.mfSells || []).filter((m: any) => inFY(m.sellDate));
    const allSells: any[] = [];
    ...
    fyStockSells.forEach((s: any) => {
      ...
      const days = s.buyDate && s.sellDate
        ? Math.max(0, Math.ceil((new Date(s.sellDate).getTime() - new Date(s.buyDate).getTime()) / 86400000))
        : 0;
      allSells.push({ id: s.id, name: s.symbol, type: "Stock", qty, buyPrice, sellPrice, buyDate: s.buyDate, sellDate: s.sellDate, profit, isLtcg: days > 365, days });
    });
    ...
    fyMfSells.forEach((m: any) => {
      ...
      const days = m.buyDate && m.sellDate
        ? Math.max(0, Math.ceil((new Date(m.sellDate).getTime() - new Date(m.buyDate).getTime()) / 86400000))
        : 0;
      allSells.push({ id: m.id, name: m.name || m.symbol, type: "Mutual Fund", qty, buyPrice: buyNav, sellPrice: sellNav, buyDate: m.buyDate, sellDate: m.sellDate, profit, isLtcg: days > 365, days });
    });

    allSells.sort((a, b) => new Date(b.sellDate).getTime() - new Date(a.sellDate).getTime());
    let stcgGains = 0, stcgLosses = 0, ltcgGains = 0, ltcgLosses = 0;
    allSells.forEach(s => {
      if (s.isLtcg) { s.profit > 0 ? (ltcgGains += s.profit) : (ltcgLosses += Math.abs(s.profit)); }
      else { s.profit > 0 ? (stcgGains += s.profit) : (stcgLosses += Math.abs(s.profit)); }
    });
    return { allSells, stcgGains, stcgLosses, ltcgGains, ltcgLosses };
  }, [state.stockSells, state.mfSells, fy]);
  ...
  const harvestCandidates = useMemo(() => {
    ...
    (state.stocks || []).forEach((s: any) => {
      ...
      if (loss > 0 && qty > 0) {
        const days = s.buyDate ? Math.max(0, Math.ceil((Date.now() - new Date(s.buyDate).getTime()) / 86400000)) : 0;
        candidates.push({ id: s.id, name: s.symbol || s.name, type: "Stock", qty, buyPrice: avgPrice, currentPrice, invested, currentVal, loss, buyDate: s.buyDate, isLtcg: days > 365, days });
      }
    });
    (state.mutualFunds || []).forEach((m: any) => {
      ...
      const loss = invested - currentVal;
      if (loss > 0 && units > 0) {
        const days = m.buyDate ? Math.max(0, Math.ceil((Date.now() - new Date(m.buyDate).getTime()) / 86400000)) : 0;
        candidates.push({ id: m.id, name: m.name || m.symbol, type: "Mutual Fund", qty: units, buyPrice: units > 0 ? invested / units : 0, currentPrice: currentNav, invested, currentVal, loss, buyDate: m.buyDate, isLtcg: days > 365, days });
      }
    });
  ...
  const taxCalculations = useMemo(() => {
    ...
    if (actualNetSTCG < 0) { actualNetLTCG = Math.max(0, actualNetLTCG + actualNetSTCG); actualNetSTCG = 0; }
    ...
    let simulatedSTCLosses = 0, simulatedLTCLosses = 0;
    ...
    if (simNetSTCG < 0) { simNetLTCG = Math.max(0, simNetLTCG + simNetSTCG); simNetSTCG = 0; }
    ...
    return {
      actual: { netSTCG: Math.max(0, actualNetSTCG), netLTCG: Math.max(0, actualNetLTCG), taxSTCG: actualTaxSTCG, taxLTCG: actualTaxLTCG, totalTax: totalActualTax },
      simulated: { stcLossesAdded: simulatedSTCLosses, ltcLossesAdded: simulatedLTCLosses, netSTCG: Math.max(0, simNetSTCG), netLTCG: Math.max(0, simNetLTCG), taxSTCG: simTaxSTCG, taxLTCG: simTaxLTCG, totalTax: totalSimTax },
  ...
  const printTaxSummary = () => {
    ...
    const lines = [
      ...
      ``,
      `TAX COMPUTATION`,
      ...r.slabs.filter((s: any) => s.incomeInSlab > 0).map((s: any) => {
        const pad = " ".repeat(Math.max(0, 30 - s.label.length));
        return `  ${s.label} (${s.range})${pad}: ${fmtINRFull(s.taxInSlab)}`;
      }),
      r.rebateApplied ? `  Section 87A Rebate        : - ${fmtINRFull(r.rebateAmount || r.tax)}` : "",
  ...
  const downloadCGCsv = () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = `capital_gains_FY${fy}.csv`; a.click();
  ...
  return (
    <div className="tab-content-enter">
      ...
      {(() => {
        const tiles = [
          { label: "Financial Year",      value: `FY ${fy}`,                              sub: "Active fiscal year",                                         color: THEME.accent,                                    Icon: Calendar     },
          hasNewRegime
            ? { label: "Active Regime", value: activeRegime === "new" ? "New Regime" : "Old Regime", sub: activeRegime === "new" ? `₹${getNewStdDed(fyStartYear)/1000}K std deduction` : "Full deductions allowed", color: THEME.gold, Icon: BookOpen }
            : { label: "Tax Regime",    value: "Old Regime",                               sub: "Single regime (pre FY 2020-21)",                             color: THEME.gold,                                      Icon: BookOpen     },
          { label: "Gross Tax Liability", value: fmtINRFull(currentTax),                  sub: `${activeRegime === "new" ? "New" : "Old"} regime · ${currentResult.effectiveRate.toFixed(1)}% eff. rate`, color: THEME.rust, Icon: Calculator },
          { label: "Paid So Far",         value: fmtINRFull(totalPaidSoFar),               sub: "TDS + Advance + Self-Assess",                                color: THEME.sage,                                      Icon: CheckCircle2 },
          { label: "Balance Due",         value: fmtINRFull(remainingAdvance),             sub: remainingAdvance <= 0 ? "Fully settled" : "Still to pay",     color: remainingAdvance > 0 ? THEME.gold : THEME.sage, Icon: AlertTriangle },
        ];
        return (
            ...
            {tiles.map(({ label, value, sub, color, Icon }) => (
              <div key={label} className="card-lift" style={{ background: "var(--surface-0)", border: `1px solid ${THEME.line}`, borderTop: `4px solid ${color}`, borderRadius: 14, padding: "18px 20px", display: "flex", flexDirection: "column", gap: 12, boxShadow: "var(--shadow-card)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: `${color}1f`, display: "flex", alignItems: "center", justifyContent: "center", color, flexShrink: 0 }}><Icon size={18} /></div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: THEME.muted, textTransform: "uppercase", letterSpacing: "0.1em" }}>{label}</div>
                </div>
                <div style={{ fontSize: 24, fontWeight: 900, color: THEME.ink, letterSpacing: "-0.03em", lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>{value}</div>
      ...
      <div className="demat-portfolio-bar no-scrollbar" style={{ marginBottom: 28 }}>
        {[
          ...
          <button key={id} className={`demat-portfolio-pill ${subTab === id ? "active" : ""}`} onClick={() => setSubTab(id as any)}>
      ...
      {subTab === "income" && (
        <div className="tab-content-enter">
          ...
          {hasNewRegime && regimeVerdict && annualIncome > 0 && (
            <div style={{ marginBottom: 28 }}>
              <Card style={{
                ...
                <div style={{ height: 3, background: regimeVerdict.better === "new" ? `linear-gradient(90deg, ${THEME.accent}, #818CF8)` : regimeVerdict.better === "old" ? `linear-gradient(90deg, ${THEME.gold}, #FBBF24)` : THEME.line }} />
                <div style={{ padding: "20px 24px" }}>
                  {/* Header */}
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: `${THEME.accent}15`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Award size={18} color={THEME.accent} />
                    </div>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 900, color: THEME.ink }}>CFO Regime Advisor</div>
                      <div style={{ fontSize: 11, color: THEME.muted }}>For FY {fy} · Based on your income & deduction profile</div>
                    </div>
                    {regimeVerdict.better !== "equal" && (
                      <div style={{ marginLeft: "auto", padding: "6px 14px", borderRadius: 99, background: regimeVerdict.better === "new" ? `${THEME.accent}15` : `${THEME.gold}15`, color: regimeVerdict.better === "new" ? THEME.accent : THEME.gold, fontSize: 12, fontWeight: 800 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", gap: 16, alignItems: "center" }}>
                    {/* New Regime */}
                    <div style={{ padding: "16px 20px", borderRadius: 12, background: regimeVerdict.better === "new" ? `${THEME.accent}10` : "rgba(128,128,128,0.03)", border: `1.5px solid ${regimeVerdict.better === "new" ? THEME.accent : THEME.line}${regimeVerdict.better === "new" ? "33" : ""}` }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>
                        {regimeVerdict.better === "new" && <CheckCircle2 size={14} color={THEME.accent} />}
                        <span style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.1em", color: regimeVerdict.better === "new" ? THEME.accent : THEME.muted }}>New Regime</span>
                      </div>
                      <div style={{ fontSize: 28, fontWeight: 900, color: regimeVerdict.better === "new" ? THEME.accent : THEME.ink, letterSpacing: "-0.03em" }}>{fmtINRFull(taxNewResult.total)}</div>
                      <div style={{ fontSize: 11, color: THEME.muted, marginTop: 6 }}>
                        ₹{getNewStdDed(fyStartYear).toLocaleString("en-IN")} std ded · No other deductions<br />
                        Taxable: {fmtINRFull(taxNewResult.taxable)} · {taxNewResult.effectiveRate.toFixed(1)}% eff. rate
                      </div>
                      {taxNewResult.rebateApplied && (
                        <div style={{ marginTop: 8, fontSize: 11, fontWeight: 700, color: THEME.sage }}>✓ 87A rebate applied</div>
                      )}
                    </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                      ...
                      <div style={{ fontSize: 11, fontWeight: 800, color: THEME.muted, padding: "4px 8px", border: `1px solid ${THEME.line}`, borderRadius: 6 }}>VS</div>
                      <div style={{ width: 1, height: 24, background: THEME.line }} />
                    </div>

                    {/* Old Regime */}
                    <div style={{ padding: "16px 20px", borderRadius: 12, background: regimeVerdict.better === "old" ? `${THEME.gold}10` : "rgba(128,128,128,0.03)", border: `1.5px solid ${regimeVerdict.better === "old" ? THEME.gold : THEME.line}${regimeVerdict.better === "old" ? "33" : ""}` }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>
                        {regimeVerdict.better === "old" && <CheckCircle2 size={14} color={THEME.gold} />}
                        <span style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.1em", color: regimeVerdict.better === "old" ? THEME.gold : THEME.muted }}>Old Regime</span>
                      </div>
                      <div style={{ fontSize: 28, fontWeight: 900, color: regimeVerdict.better === "old" ? THEME.gold : THEME.ink, letterSpacing: "-0.03em" }}>{fmtINRFull(taxOldResult.total)}</div>
                      <div style={{ fontSize: 11, color: THEME.muted, marginTop: 6 }}>
                        Total deductions: {fmtINRFull(totalOldDeductions)}<br />
                        Taxable: {fmtINRFull(taxOldResult.taxable)} · {taxOldResult.effectiveRate.toFixed(1)}% eff. rate
                      </div>
                  ...
                  {regimeVerdict.saving > 0 && (
                    <div style={{ marginTop: 16, padding: "12px 16px", borderRadius: 10, background: regimeVerdict.better === "new" ? `${THEME.accent}0f` : `${THEME.gold}0f`, border: `1px solid ${regimeVerdict.better === "new" ? THEME.accent : THEME.gold}22`, display: "flex", alignItems: "center", gap: 10 }}>
                      <Zap size={16} color={regimeVerdict.better === "new" ? THEME.accent : THEME.gold} />
                      <div style={{ flex: 1, fontSize: 13, fontWeight: 700, color: THEME.ink }}>
                        {regimeVerdict.better === "new" ? "New Regime" : "Old Regime"} saves you{" "}
                        <span style={{ color: regimeVerdict.better === "new" ? THEME.accent : THEME.gold, fontWeight: 900 }}>{fmtINRFull(regimeVerdict.saving)}</span>
                        {" "}in tax this year.
                        {regimeVerdict.better === "old" && (
                          <span style={{ fontWeight: 500, color: THEME.muted }}> Maximize 80C, 80D, and NPS deductions to stay ahead.</span>
                        )}
                        {regimeVerdict.better === "new" && (
                          <span style={{ fontWeight: 500, color: THEME.muted }}> Simplified filing, no paperwork for deduction proofs needed.</span>
                        )}
                      </div>
                      <button
                  ...
                  {regimeVerdict.saving === 0 && (
                    <div style={{ marginTop: 16, padding: "10px 14px", borderRadius: 10, background: `${THEME.accent}08`, border: `1px solid ${THEME.line}`, fontSize: 13, color: THEME.muted }}>
                      Both regimes result in equal tax. You may choose either — new regime means simpler filing (no deduction paperwork).
                    </div>
                  )}
                </div>
              </Card>
            ...
            <Card variant="hero" style={{ padding: 32, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, color: "rgba(255,255,255,0.7)", fontSize: 13, fontWeight: 500 }}>
                  <Shield size={14} /> Total tax: {fmtINRFull(currentTax)} · TDS credited: {fmtINRFull(totalTDS)}
                </div>
              </div>
              <div style={{ marginTop: 32 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "rgba(255,255,255,0.6)", fontWeight: 800, textTransform: "uppercase", marginBottom: 10 }}>
                  <span>Progress Paid</span><span>{progressPct.toFixed(0)}%</span>
                </div>
                ...
                {installments.map(inst => {
                  ...
                  return (
                    <div key={inst.q} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderRadius: 12, background: "rgba(128,128,128,0.03)", border: `1px solid ${THEME.line}` }}>
                      ...
                      {!isPaid && isPartial && <Badge variant="gold" style={{ fontSize: 9 }}>Shortfall</Badge>}
                    </div>
                  );
                })}
              </div>
          ...
          {isAdvanceTaxApplicable && (() => {
            ...
            if (allPaid) {
              return (
                <Card style={{ padding: 20, marginBottom: 24, background: `${THEME.sage}09`, border: `1px solid ${THEME.sage}33`, display: "flex", alignItems: "center", gap: 16 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: THEME.muted, marginTop: 2 }}>FY {fy} — {fmtINRFull(totalAdvancePaid)} paid of {fmtINRFull(netLiability)} net liability</div>
                  </div>
                </Card>
              );
            }
            const upcoming = instDates.find(i => totalAdvancePaid < i.cumAmt && i.dueDate >= now);
            ...
            return (
              <Card style={{ padding: 24, marginBottom: 24, border: `1.5px solid ${countColor}30` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", color: THEME.muted, marginBottom: 4 }}>Advance Tax Countdown</div>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontSize: 48, fontWeight: 900, color: countColor, letterSpacing: "-0.03em", lineHeight: 1 }}>{Math.abs(daysLeft)}</div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: countColor, textTransform: "uppercase", letterSpacing: "0.1em" }}>{isOverdue ? "days overdue" : "days left"}</div>
                  </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 16 }}>
                  {[
                    { label: "Amount Due Now", value: fmtINRFull(Math.max(0, target.cumAmt - totalAdvancePaid)), color: countColor },
                    ...
                    <div key={label} style={{ textAlign: "center", padding: "12px 8px", background: "rgba(128,128,128,0.03)", borderRadius: 10, border: `1px solid ${THEME.line}` }}>
                      <div style={{ fontSize: 10, color: THEME.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 6 }}>{label}</div>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {instDates.map(i => {
                    ...
                    return <div key={i.q} style={{ flex: 1, height: 8, borderRadius: 4, background: paid ? THEME.sage : isCurrent ? countColor : THEME.line, transition: "background 0.3s" }} title={`${i.q}: ${i.pct}%`} />;
                  })}
                </div>
                ...
                {isOverdue && (
                  <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 8, background: `${THEME.rust}0f`, border: `1px solid ${THEME.rust}26`, fontSize: 12, color: THEME.rust, fontWeight: 600 }}>
                    ⚠️ Overdue by {Math.abs(daysLeft)} days. Interest u/s 234B/234C may apply. Pay immediately via Challan 280.
                  </div>
                )}
              </Card>
            );
          ...
          {(() => {
            ...
            return (
              <div style={{ padding: "14px 20px", borderRadius: 12, background: isOverdue ? `${THEME.rust}09` : isUrgent ? `${THEME.gold}09` : `${THEME.sage}09`, border: `1.5px solid ${bannerColor}30`, display: "flex", alignItems: "center", gap: 16, marginBottom: 28 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 800, color: THEME.ink }}>
                    ITR Filing Deadline — AY {fyStartYear + 1}-{String(fyStartYear + 2).slice(-2)} · 31 July {fyStartYear + 1}
                  </div>
          ...
          <div style={{ marginBottom: 28 }}>
            <div
              ...
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", marginBottom: showBreakdown ? 16 : 0 }}
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <Button variant="secondary" size="sm" icon={<Calculator size={14} />} onClick={(e: any) => { e.stopPropagation(); printTaxSummary(); }}>
                  Tax Report
                </Button>
                <div style={{ width: 32, height: 32, borderRadius: 8, border: `1px solid ${THEME.line}`, display: "flex", alignItems: "center", justifyContent: "center", color: THEME.muted }}>
            ...
            {showBreakdown && (
              <Card style={{ padding: 28 }}>
                {annualIncome === 0 ? (
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: hasNewRegime ? "1fr 1fr" : "1fr", gap: 32 }}>
                    {/* New Regime breakdown */}
                    {hasNewRegime && (
                      <div>
                        <div style={{ fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.1em", color: THEME.accent, marginBottom: 16, display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.1em", color: THEME.gold, marginBottom: 16, display: "flex", alignItems: "center", gap: 6 }}>
                        <div style={{ width: 8, height: 8, borderRadius: 2, background: THEME.gold }} />
                        Old Regime · FY {fy}
                      </div>
                ...
                {currentResult.surcharge === 0 && (
                  <div style={{ marginTop: 16, padding: "8px 12px", borderRadius: 8, background: `${THEME.sage}0a`, border: `1px solid ${THEME.sage}22`, fontSize: 11, color: THEME.muted }}>
                    ℹ️ Surcharge (10%–37%) applies only for income above ₹50 lakh. Health & Education Cess @ 4% is mandatory on tax + surcharge.
                  </div>
                )}
              </Card>
            )}
          ...
          <div style={{ marginBottom: 16 }}>
            <h3 style={{ fontSize: 18, fontWeight: 900, margin: 0, letterSpacing: "-0.02em" }}>
              {activeRegime === "old" ? "Income & Deductions (Old Regime)" : "Income & Deductions Reference"}
            </h3>
          </div>
          <Card style={{ padding: 32, marginBottom: 28 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24, paddingBottom: 24, borderBottom: `1.5px solid ${THEME.line}` }}>
              <Field label="Annual Income (for tax computation)">
                <input className="form-input" type="number"
                  placeholder={detectedIncome > 0 ? `Auto: ${Math.round(detectedIncome).toLocaleString("en-IN")}` : "Enter annual income"}
              ...
              <div style={{ display: "flex", flexDirection: "column", justifyContent: "flex-end", paddingBottom: 2 }}>
                <div style={{ fontSize: 11, color: THEME.muted, fontWeight: 600, lineHeight: 1.6 }}>
                  {incomeOverride !== "" ? (
                    ...
                    <span>Auto-detected: <b style={{ color: THEME.ink }}>{fmtINRFull(detectedIncome)}/yr</b></span>
                  ) : (
                    <span style={{ color: THEME.rust }}>No income detected — enter annual income manually</span>
                  )}
                  {incomeOverride !== "" && (
                ...
                {annualIncome > 0 && (
                  <div style={{ marginTop: 8, fontSize: 12, fontWeight: 700, color: THEME.sage }}>
                    {activeRegime === "new"
                      ? (taxNewResult.rebateApplied && taxNewResult.total === 0 ? "✓ 87A rebate: ₹0 tax (income within threshold)" : `New regime taxable: ${fmtINRFull(taxNewResult.taxable)}`)
                      : (taxOldResult.rebateApplied && taxOldResult.total === 0 ? "✓ 87A rebate: ₹0 tax after deductions" : `Old regime taxable: ${fmtINRFull(taxOldResult.taxable)}`)}
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Deductions grid */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 8, alignItems: "center", gridColumn: "1/-1" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, gridColumn: "1/-1", marginBottom: 10, paddingBottom: 12, borderBottom: `1px solid ${THEME.line}` }}>
                <Target size={16} color={activeRegime === "old" ? THEME.gold : THEME.muted} />
                <span style={{ fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.05em", color: activeRegime === "old" ? THEME.gold : THEME.muted }}>
                  ...
                  return (
                    <div style={{ marginTop: 8 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: THEME.muted, fontWeight: 700, marginBottom: 4 }}>
                        <span style={{ color: barColor }}>{pct.toFixed(0)}% utilized · {fmtINRFull(used)} of ₹1.5L</span>
                        <span>{remaining > 0 ? `${fmtINRFull(remaining)} room left` : "Fully utilized ✓"}</span>
                      </div>
            ...
            {/* Summary bar */}
            <div style={{ marginTop: 32, paddingTop: 24, borderTop: `2px solid ${THEME.line}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
              <div>
                {hasNewRegime ? (
                  <>
                    <div style={{ fontSize: 11, color: THEME.muted, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>
                      {regimeVerdict && regimeVerdict.saving > 0
                        ? (regimeVerdict.better === "old" ? "Old Regime Saves You" : "New Regime Saves You")
                        : "Both Regimes — Same Tax"}
                    </div>
                    <div style={{ fontSize: 32, fontWeight: 900, color: THEME.sage }}>{fmtINRFull(regimeVerdict ? regimeVerdict.saving : 0)}</div>
                  ...
                  <>
                    <div style={{ fontSize: 11, color: THEME.muted, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>Tax Liability (Old Regime)</div>
                    <div style={{ fontSize: 32, fontWeight: 900, color: THEME.rust }}>{fmtINRFull(taxOldResult.total)}</div>
                    <div style={{ fontSize: 12, color: THEME.muted, marginTop: 4 }}>Effective rate: {taxOldResult.effectiveRate.toFixed(2)}%</div>
                  </>
                )}
              </div>
              <Button variant="accent" size="lg" icon={<Calculator size={20} />} style={{ padding: "12px 40px" }} onClick={printTaxSummary}>
                Generate Tax Report
              </Button>
            </div>
          </Card>
          ...
          {taxSavingTips.length > 0 && (
            <div style={{ marginBottom: 28 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                ...
                <h3 style={{ fontSize: 18, fontWeight: 900, margin: 0, letterSpacing: "-0.02em" }}>CFO Tax Saving Tips</h3>
                <Badge variant="gold" style={{ fontSize: 10, fontWeight: 800 }}>{taxSavingTips.length} Opportunities</Badge>
              </div>
              <div style={{ display: "grid", gap: 12 }}>
                {taxSavingTips.map(tip => {
                  const priorityColor = tip.priority === "high" ? THEME.rust : tip.priority === "medium" ? THEME.gold : THEME.muted;
                  const isExpanded = expandedTipId === tip.id;
                  return (
                    <div
                      key={tip.id}
                      style={{ borderRadius: 14, background: "var(--surface-0)", border: `1.5px solid ${isExpanded ? priorityColor : THEME.line}22`, overflow: "hidden", transition: "border-color 0.2s" }}
                    >
                      <div
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
                            <span style={{ fontWeight: 800, fontSize: 14, color: THEME.ink }}>{tip.title}</span>
                            <Badge variant={tip.priority === "high" ? "rust" : tip.priority === "medium" ? "gold" : "muted"} style={{ fontSize: 9 }}>
                              {tip.priority} priority
                            </Badge>
                            <Badge variant="muted" style={{ fontSize: 9 }}>
                              {tip.regime === "both" ? "Both Regimes" : tip.regime === "old" ? "Old Regime Only" : "New Regime"}
                            </Badge>
                          </div>
                        ...
                        <div style={{ textAlign: "right", flexShrink: 0 }}>
                          {tip.saving != null && tip.saving > 0 && (
                            <div>
                              <div style={{ fontSize: 10, color: THEME.muted, fontWeight: 700, textTransform: "uppercase", marginBottom: 2 }}>Potential Saving</div>
                              <div style={{ fontSize: 16, fontWeight: 900, color: THEME.sage }}>~{fmtINRFull(tip.saving)}</div>
                            </div>
                          )}
                      ...
                      {isExpanded && (
                        <div style={{ padding: "0 20px 20px", borderTop: `1px solid ${THEME.line}` }}>
                          <div style={{ marginTop: 12, padding: "14px 16px", borderRadius: 10, background: `${priorityColor}08`, border: `1px solid ${priorityColor}18` }}>
                            <div style={{ fontSize: 11, fontWeight: 700, color: priorityColor, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>{tip.section}</div>
                            <div style={{ fontSize: 13, color: THEME.muted, lineHeight: 1.7 }}>{tip.fullDesc}</div>
                            {tip.maxBenefit != null && tip.maxBenefit > 0 && (
                              <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 700, color: THEME.sage }}>
                                <Zap size={13} />
                            ...
                            {tip.utilizedPct > 0 && (
                              <div style={{ marginTop: 10 }}>
                                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: THEME.muted, fontWeight: 700, marginBottom: 4 }}>
                                  <span>Utilization</span><span>{tip.utilizedPct.toFixed(0)}%</span>
                                </div>
                                <div style={{ height: 4, background: THEME.line, borderRadius: 4, overflow: "hidden" }}>
                                  <div style={{ height: "100%", width: `${Math.min(100, tip.utilizedPct)}%`, background: priorityColor, borderRadius: 4 }} />
                                </div>
                              </div>
                            )}
                          </div>
          ...
          {taxPayments.length === 0 ? (
            <Card style={{ padding: "40px 0", textAlign: "center", color: THEME.muted, fontSize: 13, marginBottom: 40 }}>
              No tax payments or TDS entries recorded yet. Use 'Record Payment' to track advance tax and TDS credits.
            </Card>
          ) : (
                ...
                <Card key={p.id} style={{ padding: "16px 20px", borderTop: `4px solid ${p.type === "TDS" ? THEME.gold : THEME.sage}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: THEME.muted, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.date} · {p.note || "No note"}</div>
                    </div>
      ...
      {subTab === "capitalGains" && (
        <div className="tab-content-enter">
          ...
          <div style={{ padding: "10px 16px", borderRadius: 10, background: `${THEME.accent}09`, border: `1px solid ${THEME.accent}22`, display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 20, fontSize: 12, color: THEME.muted }}>
            <Info size={15} color={THEME.accent} style={{ flexShrink: 0, marginTop: 1 }} />
            <span>
              <b style={{ color: THEME.ink }}>Post-Budget 2024 rates (effective 23 Jul 2024):</b> STCG on listed equity / equity MF @ <b>20%</b> · LTCG @ <b>12.5%</b> with ₹1.25L annual exemption.
              {" "}<b>Debt MF</b> (purchased after 1 Apr 2023) — gains are taxed at slab rate regardless of holding period.
            </span>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 20, marginBottom: 32 }}>
            <Card variant="hero" style={{ padding: 32, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, color: "rgba(255,255,255,0.7)", fontSize: 13, fontWeight: 500 }}>
                  <Percent size={14} /> Actual: {fmtINRFull(taxCalculations.actual.totalTax)} · Simulated: {fmtINRFull(taxCalculations.simulated.totalTax)}
                </div>
              </div>
              <div style={{ marginTop: 32 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "rgba(255,255,255,0.6)", fontWeight: 800, textTransform: "uppercase", marginBottom: 10 }}>
                  <span>Offset Achieved</span>
                  <span>{taxCalculations.actual.totalTax > 0 ? `${Math.min(100, (taxCalculations.totalSaved / taxCalculations.actual.totalTax) * 100).toFixed(0)}%` : "0%"}</span>
                </div>
                <div style={{ height: 8, background: "rgba(255,255,255,0.15)", borderRadius: 10, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: taxCalculations.actual.totalTax > 0 ? `${Math.min(100, (taxCalculations.totalSaved / taxCalculations.actual.totalTax) * 100)}%` : "0%", background: "#34D399", boxShadow: "0 0 15px rgba(52,211,153,0.5)", borderRadius: 10, transition: "width 0.4s" }} />
                </div>
                <div style={{ display: "flex", gap: 24, marginTop: 16 }}>
                  <div>
                    <div style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", textTransform: "uppercase" }}>Net STCG (Actual)</div>
                    <div style={{ fontSize: 15, fontWeight: 800, color: "#fff" }}>
                      {fmtINRFull(taxCalculations.actual.netSTCG)} <span style={{ fontSize: 11, color: "rgba(255,255,255,0.6)" }}>(Tax: {fmtINRFull(taxCalculations.actual.taxSTCG)})</span>
                    </div>
                  </div>
                  <div style={{ width: 1, background: "rgba(255,255,255,0.1)" }} />
                  <div>
                    <div style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", textTransform: "uppercase" }}>Net LTCG (Actual)</div>
                    <div style={{ fontSize: 15, fontWeight: 800, color: "#fff" }}>
                      {fmtINRFull(taxCalculations.actual.netLTCG)} <span style={{ fontSize: 11, color: "rgba(255,255,255,0.6)" }}>(Tax: {fmtINRFull(taxCalculations.actual.taxLTCG)})</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <Card style={{ padding: 24, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  ...
                  {simulatedHarvestIds.length > 0 && (
                    <Button variant="ghost" size="sm" onClick={() => setSimulatedHarvestIds([])} icon={<RefreshCw size={12} />} style={{ fontSize: 11, padding: "2px 8px", color: THEME.rust, background: `${THEME.rust}15` }}>
                      Reset
                    </Button>
                  )}
                </div>
                <div style={{ display: "grid", gap: 14 }}>
                  {[
                    { label: "Securities Harvested", value: `${simulatedHarvestIds.length} Assets` },
                    { label: "Simulated STCL Applied", value: `-${fmtINRFull(taxCalculations.simulated.stcLossesAdded)}` },
                    { label: "Simulated LTCL Applied", value: `-${fmtINRFull(taxCalculations.simulated.ltcLossesAdded)}` },
                  ].map(({ label, value }) => (
                    <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: 12, borderBottom: `1px solid ${THEME.line}` }}>
                      <span style={{ fontSize: 13, fontWeight: 600, color: THEME.muted }}>{label}</span>
                      <span style={{ fontSize: 14, fontWeight: 800, color: THEME.ink }}>{value}</span>
                    </div>
                  ))}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: THEME.ink }}>Simulated Net Tax</span>
              ...
              {taxCalculations.actual.totalTax === 0 && (
                <div style={{ marginTop: 16, padding: "10px 12px", borderRadius: 10, background: `${THEME.accent}0f`, border: `1px solid ${THEME.accent}26`, fontSize: 11, fontWeight: 600, color: THEME.accent, lineHeight: 1.5 }}>
                  <Info size={14} style={{ float: "left", marginRight: 6, marginTop: 1 }} />
                  No realized capital gains this FY. Simulated losses can be carried forward for up to 8 assessment years to offset future profits.
                </div>
              )}
            </Card>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div>
              ...
              <p style={{ fontSize: 12, color: THEME.muted, marginTop: 2 }}>Select holdings trading at unrealized loss to simulate selling and offsetting realized gains.</p>
            </div>
            <Badge variant="muted" style={{ fontWeight: 800 }}>{harvestCandidates.length} Loss Candidates</Badge>
          </div>

          {harvestCandidates.length === 0 ? (
            <Card style={{ padding: "40px 0", textAlign: "center", color: THEME.muted, fontSize: 13, marginBottom: 32 }}>
              🎉 None of your current holdings are trading at a loss. No harvesting offset needed this FY!
            </Card>
          ) : (
            <div style={{ display: "grid", gap: 12, marginBottom: 32 }}>
              {harvestCandidates.map(cand => {
                const isSelected = simulatedHarvestIds.includes(cand.id);
                const taxSavedEst = cand.isLtcg ? cand.loss * 0.125 : cand.loss * 0.20;
                return (
                  <div key={cand.id} onClick={() => setSimulatedHarvestIds(prev => prev.includes(cand.id) ? prev.filter(x => x !== cand.id) : [...prev, cand.id])}
                    style={{ display: "flex", alignItems: "center", gap: 16, padding: "16px 20px", borderRadius: 14, background: isSelected ? `${THEME.sage}09` : "var(--surface-0)", border: `1.5px solid ${isSelected ? THEME.sage : THEME.line}`, cursor: "pointer", transition: "all 0.2s" }}>
                    <input type="checkbox" checked={isSelected} onChange={() => {}} style={{ width: 18, height: 18, accentColor: "var(--t-sage)", cursor: "pointer" }} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        ...
                        <Badge variant={cand.isLtcg ? "muted" : "gold"} style={{ fontSize: 9 }}>{cand.isLtcg ? "LTCG (Long-Term)" : "STCG (Short-Term)"}</Badge>
                      </div>
                      <div style={{ fontSize: 11, color: THEME.muted, fontWeight: 600, marginTop: 4 }}>
                        Bought: {cand.buyDate || "—"} · Qty: {cand.qty.toFixed(2)} · Cost: {fmtINRFull(cand.invested)} · Market Val: {fmtINRFull(cand.currentVal)}
                      </div>
                    </div>
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      <div style={{ fontSize: 11, color: THEME.muted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.03em" }}>Unrealized Loss</div>
                      <div style={{ fontSize: 16, fontWeight: 900, color: THEME.rust, marginTop: 2 }}>{fmtINRFull(cand.loss)}</div>
                      <div style={{ fontSize: 10, color: THEME.sage, fontWeight: 700, marginTop: 2 }}>Offsets ~{fmtINRFull(taxSavedEst)} tax</div>
                    </div>
                  </div>
                );
              })}
          ...
          <Card style={{ padding: 24, background: `${THEME.accent}09`, border: `1.5px solid ${THEME.accent}22`, marginBottom: 24 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 800, color: THEME.accent, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>Indian Statutory Netting Rules</div>
                <ul style={{ fontSize: 12, color: THEME.muted, margin: 0, paddingLeft: 16, lineHeight: 1.7 }}>
                  <li><b>STCL</b> can offset both Short-Term and Long-Term Capital Gains (Sec 70).</li>
                  <li style={{ marginTop: 4 }}><b>LTCL</b> can only offset Long-Term Capital Gains — cannot touch STCG (Sec 70).</li>
                  <li style={{ marginTop: 4 }}><b>Carry Forward:</b> Unabsorbed CG losses can be carried forward for up to <b>8 assessment years</b> (ITR must be filed before due date to preserve carry-forward).</li>
                  <li style={{ marginTop: 4 }}><b>LTCG exemption:</b> First ₹1.25L of LTCG on equity / equity MF per year is exempt u/s 112A.</li>
                </ul>
              </div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 800, color: THEME.gold, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8, display: "flex", alignItems: "center", gap: 4 }}>
                  <AlertTriangle size={13} /> CFO Advisory — Avoid Wash-Sales
                </div>
                <p style={{ fontSize: 12, color: THEME.muted, margin: 0, lineHeight: 1.7 }}>
                  Selling purely to book tax losses and repurchasing the same asset the same day can invite audit scrutiny under GAAR (General Anti-Avoidance Rule, Sec 96-102). To avoid tax evasion flags: wait at least 2-3 business days before re-entering, or reinvest in a highly correlated alternative (different ETF / similar sector MF) to lock in the tax saving safely.
                </p>
              </div>
            </div>
          </Card>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              {realizedGainsData.allSells.length > 0 && (
                <button onClick={downloadCGCsv} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", borderRadius: 8, border: `1px solid ${THEME.sage}55`, background: "transparent", color: THEME.sage, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
          ...
          {realizedGainsData.allSells.length === 0 ? (
            <Card style={{ padding: "40px 0", textAlign: "center", color: THEME.muted, fontSize: 13, marginBottom: 40 }}>
              No realized stock or mutual fund sales in FY {fy} yet. Add sales in the Demat / Mutual Fund trackers.
            </Card>
          ) : (
            <div style={{ background: "var(--surface-0)", borderRadius: 14, border: `1px solid ${THEME.line}`, overflow: "hidden", marginBottom: 40 }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, textAlign: "left" }}>
                <thead>
                  <tr style={{ background: "rgba(128,128,128,0.03)", borderBottom: `1px solid ${THEME.line}` }}>
                    {["Asset Name", "Type", "Holding Days", "Buy Price", "Sell Price", "Gain / Loss"].map(h => (
                      <th key={h} style={{ padding: "14px 20px", fontWeight: 800, color: THEME.muted, textAlign: ["Holding Days", "Buy Price", "Sell Price", "Gain / Loss"].includes(h) ? "right" : "left" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {realizedGainsData.allSells.map((sell, idx) => {
                    ...
                    return (
                      <tr key={sell.id || idx} style={{ borderBottom: idx < realizedGainsData.allSells.length - 1 ? `1px solid ${THEME.line}` : "none" }} className="hover-row">
                        ...
                        <td style={{ padding: "14px 20px" }}><Badge variant={sell.isLtcg ? "muted" : "gold"}>{sell.isLtcg ? "LTCG" : "STCG"}</Badge></td>
                        <td style={{ padding: "14px 20px", textAlign: "right", fontWeight: 600, color: THEME.muted }}>{sell.days} Days</td>
                        <td style={{ padding: "14px 20px", textAlign: "right", fontWeight: 600 }}>{fmtINRFull(sell.buyPrice)}</td>
                        <td style={{ padding: "14px 20px", textAlign: "right", fontWeight: 600 }}>{fmtINRFull(sell.sellPrice)}</td>
      ...
      {showModal && <AddTaxPaymentModal onClose={() => setShowModal(false)} onSave={(data: any) => { addItem("taxPayments", data); setShowModal(false); }} />}
    </div>
  );
};
