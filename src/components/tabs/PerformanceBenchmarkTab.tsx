...
const BENCHMARKS = {
  ...
  niftyMidcap: { label: "Nifty Midcap 150", return1Y: 22, return3Y: 18, return5Y: 20, color: "#10B981" },
  fdRate: { label: "FD Rate (SBI)", return1Y: 7.1, return3Y: 6.5, return5Y: 6.8, color: "#F59E0B" },
  inflation: { label: "Inflation (CPI)", return1Y: 5.5, return3Y: 5.8, return5Y: 5.5, color: "#EF4444" },
...
export const PerformanceBenchmarkTab = ({ state, metrics, marketData }) => {
  ...
  const portfolioReturns = useMemo(() => {
    ...
    const avgFDRate = fds.length > 0 ? fds.reduce((s, f) => s + Number(f.rate || 0), 0) / fds.length : 0;
  ...
  const assetComparison = useMemo(() => {
    return [
      { category: "Equity", yours: portfolioReturns.equity.return, benchmark: BENCHMARKS.nifty50.return1Y },
      { category: "Mutual Funds", yours: portfolioReturns.mf.return, benchmark: BENCHMARKS.nifty50.return1Y },
      { category: "Fixed Deposits", yours: portfolioReturns.fd.rate, benchmark: BENCHMARKS.fdRate.return1Y },
      { category: "PPF", yours: portfolioReturns.ppf.rate, benchmark: BENCHMARKS.ppf.return1Y },
      { category: "Gold", yours: portfolioReturns.gold.return, benchmark: BENCHMARKS.gold.return1Y },
    ].filter((a) => a.yours !== 0 || a.benchmark !== 0);
  }, [portfolioReturns]);

  // Financial health radar
  const healthScore = useMemo(() => {
    const savingsRate = metrics.monthIncome > 0 ? (1 - metrics.monthExpense / metrics.monthIncome) * 100 : 0;
    const debtRatio = metrics.debtToAssetRatio || 0;
    const emergencyMonths = metrics.monthExpense > 0
      ? ((state.bankAccounts || []).reduce((s, a) => s + Number(a.balance || 0), 0)) / metrics.monthExpense
      : 0;
    ...
    return [
      ...
      { metric: "Returns vs FD", score: Math.min(100, Math.max(0, (portfolioReturns.overall.return / 7) * 50)), fullMark: 100 },
