...
function weekRange() {
  ...
  const mon = new Date(ist); mon.setUTCDate(ist.getUTCDate() - ((day + 6) % 7));
...
function computeSummary(state) {
  ...
  const mfTotal = (state.mutualFunds || []).reduce((s, x) => s + (Number(x.currentValue || x.invested) || 0), 0);
  const stockTotal = (state.stocks || []).reduce((s, x) => s + ((Number(x.qty) || 0) * (Number(x.currentPrice || x.avgPrice || x.buyPrice) || 0)), 0);
  const fdTotal = (state.fixedDeposits || []).reduce((s, x) => s + (Number(x.principal) || 0), 0);
  const rdTotal = (state.recurringDeposits || []).reduce((s, x) => s + (Number(x.balance || x.invested || x.principal) || 0), 0);
  const ppfTotal = (state.ppf || []).reduce((s, x) => s + (Number(x.currentValue || x.balance || x.invested) || 0), 0);
  const npsTotal = (state.nps || []).reduce((s, x) => s + (Number(x.currentValue || x.balance || x.invested) || 0), 0);
  const epfTotal = (state.epf || []).reduce((s, x) => s + (Number(x.balance || x.currentValue || x.invested) || 0), 0);
  const bondsTotal = (state.bonds || []).reduce((s, x) => s + (Number(x.faceValue || x.invested) || 0), 0);
  ...
  activeCards.forEach(c => {
    ...
    if (d.getTime() >= todayMs && d.getTime() <= in7Ms) {
      dues.push({ date: d, label: `${c.issuer} CC Bill`, amount: Number(c.outstanding) || 0, type: "cc" });
    }
  });
  ...
  (state.loansTaken || []).forEach(l => {
    ...
    if (d.getTime() < todayMs) d.setUTCMonth(d.getUTCMonth() + 1);
    if (d.getTime() >= todayMs && d.getTime() <= in7Ms) {
      dues.push({ date: d, label: `${l.lender || l.lenderBorrower || "Loan"} EMI`, amount: Number(l.emi) || 0, type: "emi" });
    }
  });
  ...
  (state.reminders || []).filter(r => !r.done).forEach(r => {
    ...
    if (d.getTime() >= todayMs && d.getTime() <= in7Ms) {
      dues.push({ date: d, label: r.title || r.note || "Reminder", amount: Number(r.amount) || 0, type: "reminder" });
    }
  });
  ...
  // ── Alerts ────────────────────────────────────────────────────────────────
  const alerts = [];
  if (creditUtil >= 70) alerts.push({ type: "warn", msg: `Credit utilization at ${creditUtil}% — consider paying down outstanding` });
  budgetStatus.filter(b => b.over).forEach(b =>
    alerts.push({ type: "warn", msg: `${b.category} budget exceeded: spent ${fmtINR(b.spent)} of ${fmtINR(b.limit)} (${b.pct}%)` })
  );
  const overdueDues = dues.filter(d => d.date.getTime() < todayMs);
  if (overdueDues.length) alerts.push({ type: "alert", msg: `${overdueDues.length} overdue payment${overdueDues.length > 1 ? "s" : ""} need attention` });
  ...
  return {
    ...
    activeCardCount: activeCards.length, activeCards,
  };
}
...
function generateHTML(summary, frequency, recipientName) {
  ...
  const periodLabel = frequency === "weekly" ? `Week of ${weekRange()}` : frequency === "monthly" ? monthLabel() : dateStr;
  ...
  // ── Upcoming dues rows ────────────────────────────────────────────────────
  const dueRows = dues.slice(0, 6).map(d => {
    const icon = d.type === "cc" ? "💳" : d.type === "emi" ? "🏠" : d.type === "sub" ? "📱" : "📌";
  ...
  const ccRows = activeCards.slice(0, 4).map(c => {
...
async function fetchStateFromSupabase(supabase, userId) {
  ...
  return {
    ...
    creditCards: (ccs.data || []).map(c => ({ ...c, limit: c.card_limit ?? c.limit, outstanding: c.outstanding })),
